...
> **📌 项目阶段说明**：KLC 当前处于**早期原型阶段**，仍在快速迭代中。部分功能（如原生代码生成、AI 推理模块）为实验性实现，API 和行为可能在后续版本中发生较大变化。欢迎开发者参与测试和反馈，但不建议在生产环境中部署关键业务。
...
**KLC 可执行文件（klc.exe）的核心编译运行时（Lexer/Parser/Codegen/VM）零外部依赖**，不需要安装任何运行时库；示例打包工具使用 `embed-resource` 嵌入资源。
...
# 验证安装
klc version
# 输出: KLC v1.4.6-β — Kaleidoscope Language Compiler
...
```powershell
klc version
# 预期：KLC v1.4.6-β — Kaleidoscope Language Compiler
```
...
KLC v1.4.6-β 实现了完整的编译流水线，从源码到可执行文件的全链路覆盖：
...
KLC v1.4.6-β 支持 13 线程并行计算（主要用于矩阵运算模块）：
...
KLC AI 模块是集成在 KLC 语言运行时中的**实验性 AI 推理模块（基础矩阵运算和前向推理实现）**，旨在探索 KLC 语言在 AI 推理与训练场景中的应用潜力。
...
| **v1.4.6-β** | **2026-06-11** | **多标签 IDE + NMHDR 修复** |
...
*KLC v1.4.6-β 官方发行说明书 — 2026年6月11日*
