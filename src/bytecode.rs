...
pub enum Value {
    ...
    Map(Rc<RefCell<HashMap<String, Value>>>),    Matrix(Rc<RefCell<Vec<Vec<f64>>>>),
    ...
    fn eq(&self, other: &Self) -> bool {
        match (self, other) {
            ...
            (Value::Struct(a), Value::Struct(b)) => {
                let sa = a.borrow();
                let sb = b.borrow();
                sa.0 == sb.0 && sa.1.len() == sb.1.len()
                    && sa.1.iter().zip(sb.1.iter()).all(|((ka, va), (kb, vb))| ka == kb && va == vb)
        ...
        match self {
            ...
            Value::Float(n) => {
                if n.fract() == 0.0 && n.abs() < 1e15 {
                    write!(f, "{}.0", n) // 显示为 42.0 而非 42，区分 Integer
                } else {
                    write!(f, "{:.6}", n)
                }
            }
            ...
            Value::Bool(b) => write!(f, "{}", b),
            Value::Char(c) => write!(f, "{}", c),
            Value::Null => write!(f, "null"),            Value::Matrix(data) => {
                let m = data.borrow();
...
pub struct BytecodeProgram {
    ...
    /// 函数别名映射: alias_name → function_index
    /// 用于 impl 块的 Type::method / Type.method 写法
    #[doc(hidden)]
    pub fn_aliases: HashMap<String, usize>,
}
