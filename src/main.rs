...
/// v1.0.4: Token 类型 → 可读名（前置，供 run_vm 使用）
...
fn run_vm(file_path: &str, debug: bool) {
    ...
    let mut vm = VM::new(bytecode);
    // v1.0.4: VM 内部已输出中文错误详情，此处仅做退出处理
    if let Err(_) = vm.run() {
        process::exit(1);
    }
}
