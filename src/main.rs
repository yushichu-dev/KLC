...
fn main() {
    ...
    match subcmd {
        ...
        // ─── 子命令: klc version ───
        "version" | "--version" | "-v" => {
            println!("KLC v1.3.2 — Kaleidoscope Language Compiler (with JIT)");
        }
        // ─── 子命令: klc help ───
...
fn print_usage() {
    eprintln!("KLC v1.3.2 — Kaleidoscope Language Compiler (with JIT)");
    eprintln!();
    eprintln!("USAGE:");
...
/// v1.3.2: Token 类型 → 可读名（前置，供 run_vm 使用）
...
fn run_vm_with_jit(file_path: &str, debug: bool, jit_config: jit::JitConfig) {
    ...
    // v1.3.2: VM 内部已输出中文错误详情，此处仅做退出处理
    if let Err(_) = result {
        process::exit(1);
    }
