...
<<<<<<< HEAD
            ...
            major_subsystem_version:    6,           // 子系统版本 6.1 (Win7+)
            minor_subsystem_version:    1,
            win32_version_value:        0,
            // SizeOfImage: headers(0x1000) + .text(0x1000) + [.idata(0x1000)] + [.reloc(0x1000)]
            size_of_image:              (0x2000 + ((code_size.max(1) + 0xFFF) & !0xFFF)),
            size_of_headers:            0x0200,      // 512B（文件对齐）
            checksum:                   0,           // 稍后由 build() 计算
            subsystem:                  0x0003,      // WINDOWS_CUI
            // DLL特性: NX_COMPAT | DYNAMIC_BASE | HIGH_ENTROPY_VA | TERMINAL_SERVER_AWARE
            // NX_COMPAT (0x0100): 数据执行保护 (DEP) — WinXP SP2+
            // DYNAMIC_BASE (0x0040): ASLR 地址随机化 — WinVista+
...
<<<<<<< HEAD
        ...
        let reloc_file_size = if has_reloc {
            (reloc_data.len() as u32 + FILE_ALIGNMENT - 1) & !(FILE_ALIGNMENT - 1)
        } else { 0 };

        // SizeOfImage = headers(0x1000) + .text(0x1000) + [.idata(0x1000)] + [.reloc(0x1000)]
        let mut section_count = 1u32; // headers
        section_count += 1; // .text
        if has_imports { section_count += 1; }
...
<<<<<<< HEAD
        ...
        for i in 0..16 {
            if i == 1 && has_imports {
                let dd = DataDirectory { virtual_address: self.import_rva, size: self.import_size };
                self.write_struct(&mut buf, &dd);
            } else if i == 5 && has_reloc {
                // IMAGE_DIRECTORY_ENTRY_BASERELOC = 5
                let dd = DataDirectory { virtual_address: self.reloc_rva, size: reloc_data.len() as u32 };
                self.write_struct(&mut buf, &dd);
            } else if i == 10 {
                // IMAGE_DIRECTORY_ENTRY_LOAD_CONFIG = 10
                // 提供最小加载配置目录以兼容 Win10+ 安全缓解特性
...
<<<<<<< HEAD
        ...
        if has_imports {
            ...
            let idata_section = SectionHeader::idata_section(
                self.import_data.as_ref().unwrap().len() as u32, self.idata_rva, FILE_ALIGNMENT, idata_raw_offset);
            self.write_struct(&mut buf, &idata_section);
        }

        // .reloc 节
        if has_reloc {
            let reloc_raw_offset = SECTION_DATA_OFFSET + file_aligned_code_size + import_file_size;
            let reloc_section = Self::build_reloc_section_header(reloc_data.len() as u32, self.reloc_rva, FILE_ALIGNMENT, reloc_raw_offset);
            self.write_struct(&mut buf, &reloc_section);
        }

        // ---- 8. 头部对齐填充 ----
        while (buf.len() as u32) < SECTION_DATA_OFFSET { buf.push(0); }

        // ---- 9. .text 节数据 + Load Config + 对齐 ----
        let mut text_buf = self.code.clone();
        // 追加最小加载配置目录（64字节占位 + 76字节结构体 = 140字节）
        if text_buf.len() + 140 <= file_aligned_code_size as usize {
            Self::append_load_config(&mut text_buf);
        }
...
<<<<<<< HEAD
        ...
        if has_reloc {
            ...
            let reloc_end = SECTION_DATA_OFFSET + file_aligned_code_size + import_file_size + reloc_file_size;
            while (buf.len() as u32) < reloc_end { buf.push(0); }
        }

        // ---- 12. 计算并回填 PE 校验和 ----
        let checksum = Self::compute_pe_checksum(&buf);
        let checksum_offset = PE_SIGNATURE_OFFSET as usize + 4 + mem::size_of::<CoffHeader>() + 64; // offset of checksum in optional header
    ...
    /// 生成基址重定位表（.reloc 节数据）
    /// 格式: 多个 IMAGE_BASE_RELOCATION 块，每个块以 12 字节页对齐
    /// 每页包含多个 2 字节的 Type/Offset 条目
    fn build_reloc_table(_image_base: u64, _code_size: u32) -> Vec<u8> {
        let mut data = Vec::new();

        // 简化的 .reloc 表：为重定位 .text 节中的地址引用提供基础支持
        // 页 RVA = 0x1000 (.text 节起始)
        let page_rva: u32 = 0x1000;
        // 块大小 = 8(header) + 2 * num_entries
        // 最小：提供空块以表示 .reloc 节存在
        let block_size: u32 = 8; // 只有头，无条目

        // 每个 IMAGE_BASE_RELOCATION 块:
        //   u32 PageRVA (页起始 RVA)
        //   u32 BlockSize (包含头部的总大小)
        //   u16[] TypeOffset (高4位=类型, 低12位=页内偏移)
        data.extend_from_slice(&page_rva.to_le_bytes());
        data.extend_from_slice(&block_size.to_le_bytes());

        // 如果有实际需要重定位的地址，在此添加条目:
        // Type: IMAGE_REL_BASED_DIR64 (10) = 0xA000
        // 例如: data.extend_from_slice(&(0xA000u16 | offset).to_le_bytes());

        data
    }

    /// 创建 .reloc 节表头
    fn build_reloc_section_header(data_len: u32, rva: u32, file_align: u32, raw_offset: u32) -> SectionHeader {
        let raw_size = (data_len + file_align - 1) & !(file_align - 1);
        SectionHeader {
            name: *b".reloc\0\0",
            virtual_size: data_len,
            virtual_address: rva,
            ...
            number_of_relocations: 0,
            number_of_linenumbers: 0,
            // INITIALIZED_DATA | MEM_READ — 只读数据节
            characteristics: 0x4000_0040,
        }
    }

    /// 追加最小 IMAGE_LOAD_CONFIG_DIRECTORY64 到 .text 节末尾
    /// 此目录为 Win10+ 加载器所检查，提供安全缓解功能的配置
    fn append_load_config(buf: &mut Vec<u8>) {
        // IMAGE_LOAD_CONFIG_DIRECTORY64 结构体 (140 字节)
        // Size=140, TimeDateStamp=0, Major/MinorVersion=0
        // 其他所有字段默认 0 (表示禁用 CFG/RFG 等高级安全特性)
        let mut lc = Vec::with_capacity(140);
        lc.extend_from_slice(&140u32.to_le_bytes());  // Size
        lc.extend_from_slice(&0u32.to_le_bytes());    // TimeDateStamp
        lc.extend_from_slice(&0u16.to_le_bytes());    // MajorVersion
        lc.extend_from_slice(&0u16.to_le_bytes());    // MinorVersion
        // 剩余 128 字节全零 (GlobalFlagsClear, GlobalFlagsSet, CriticalSectionTimeout, etc.)
        while lc.len() < 140 { lc.push(0); }
        buf.extend_from_slice(&lc);
    }

    /// 计算 PE 文件校验和 (IMAGE_OPTIONAL_HEADER::CheckSum)
    /// 算法: 将整个文件按 16 位字累加，结果加上文件长度
    fn compute_pe_checksum(data: &[u8]) -> u32 {
        let mut sum: u64 = 0;
        let len = data.len();
