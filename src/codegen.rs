...
pub struct Codegen {
    constants: Vec<Value>,
    /// 常量去重哈希表: partial_hash → Vec<(index, full_match_flag)>
    constant_index: HashMap<u64, Vec<usize>>,
    next_label: usize,
    lambda_count: usize,
...
impl Codegen {
    ...
    /// 快速类型指纹: Value 的变体类型 + 简单字段哈希
    fn value_fingerprint(value: &Value) -> u64 {
        match value {
            Value::Integer(n) => 0u64.wrapping_add(*n as u64),
            Value::Float(f) => 1u64.wrapping_add(f.to_bits()),
            Value::Bool(b) => 2u64.wrapping_add(if *b { 1 } else { 0 }),
            Value::Null => 3,
            Value::Char(c) => 4u64.wrapping_add(*c as u64),
            Value::String(s) => {
    ...
    fn add_constant(&mut self, value: Value) -> usize {
        // 快速哈希指纹去重: 先用指纹缩小搜索范围，再做完整比较
        let fp = Self::value_fingerprint(&value);
        if let Some(indices) = self.constant_index.get(&fp) {
            for &idx in indices {
                if self.constants[idx] == value {
                    return idx;
                }
            }
        }
        ...
        self.constant_index.entry(fp).or_default().push(pos);
        pos
    }
                ...
                Stmt::ImplBlock { type_name, methods } => {
                    for method in methods {
                        if let Stmt::FnDef { name, params, body, .. } = method {
                            ...
                            functions.push(compiled);
                            // 注册 Type::method 别名 (关联函数写法1)
                            let prefixed = format!("{}::{}", type_name, name);
                            fn_aliases.insert(prefixed, func_idx);
                            // 注册 Type.method 别名 (关联函数写法2) — v0.8.4/v1.0.3 双写法兼容（当前 v1.4.6-β）
                            let dotted = format!("{}.{}", type_name, name);
                            fn_aliases.insert(dotted, func_idx);
                        }
                    }
                }
    ...
    fn compile_stmt(&mut self, stmt: &Stmt, code: &mut Vec<Instruction>) -> Result<(), String> {
        match stmt {
            ...
            Stmt::For { var, iterable, body } => {
                // for var in iterable { body }                // v1.0.3 增强: 支持 range start..end、数组遍历（当前 v1.4.6-β）
        ...
        match expr {
            ...
            Expr::Call(name, args) => {
                if name == "to_str" && args.len() == 1 {
                    ...
                    code.push(Instruction::ToString);                } else if name == "read_line" && args.is_empty() {
