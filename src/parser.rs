    ...
    fn parse_statement(&mut self) -> Result<Stmt, String> {
        match self.peek().kind {
            ...
            TokenKind::Pub | TokenKind::Any => {
                // v1.0.3: pub 作为可见性修饰符前缀（当前 v1.4.6-β）
    ...
    fn parse_call(&mut self) -> Result<Expr, String> {
        ...
        loop {
            if self.peek().kind == TokenKind::LBracket {
                ...
                let method_or_variant = method_token.lexeme.clone();
                let is_enum_variant = method_or_variant.chars().next()
                    .map_or(false, |c| c.is_uppercase());

                if self.peek().kind == TokenKind::LParen {
                    // 有参数: Type::Variant(args...) 或 Type::method(args...)
                    self.advance(); // 跳过 (
                    let mut args = Vec::new();
                    if self.peek().kind != TokenKind::RParen {
                        loop {
    ...
    fn parse_match_arm(&mut self) -> Result<MatchArm, String> {
        ...
        let body = if self.peek().kind == TokenKind::LBrace {
            ...
            match self.peek().kind {
                ...
                TokenKind::Break => {
                    self.advance();
                    vec![Stmt::Break] // v1.0.5: 修复，不再映射为 Return
                }
                _ => {
